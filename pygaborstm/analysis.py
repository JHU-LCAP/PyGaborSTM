...
def plot_mtf_contour(
    ...
    for ax, vals, rates, name, is_up in [
        ...
        cf = ax.contourf(x_smooth, y_smooth, vals_smooth,
                         levels=lvls, cmap=cmap, vmin=d["vmin"], vmax=d["vmax"])
        cs = ax.contour(x_smooth, y_smooth, vals_smooth,
                        levels=lvls, colors="k", linewidths=0.5)
...
def plot_mtf_heatmap(
    ...
    d = _get_plot_data(rsf_dict, normalize)
    fmt = ".2f" if normalize else (".1e" if d["vmax"] > 100 or d["vmax"] < 0.01 else ".2f")
    ...
    for ax, vals, rates, name, is_up in [
        ...
        sns.heatmap(
            vals, ax=ax, cmap=cmap, annot=annot, fmt=fmt,
...
def plot_mtf_1d(
    ...
    ax2.plot(range(n_up, n_up + len(temp_down)), temp_down, "v--g", label="downward", markersize=6)
